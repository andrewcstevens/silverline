...
2. **Path allowlist (exact regex shapes).** Only these are proxied:
   ...
   - `markets/{ticker}/{orderbook|candlesticks}`
   ...
   don't match an allowed shape.
3. **Query-param stripping.** Only allowlisted params per path are forwarded; any
   extra param is dropped (prevents param injection).
...
5. **CORS origin allowlist.** Only `silverline.global` (+ vercel preview) is
   allowed; the `Origin` header is reflected only if it matches. A browser Origin
   not on the list is rejected (403) even with a valid token — so only Andrew's
   browser is served. Non-browser callers (curl, ops) have no Origin header and are
   allowed via the token gate.
...
A live read-only smoke (real fetch against `external-api.kalshi.com`, test token)
was also run and returned real data for `historical/cutoff` and `series/KXBTC15M`.
...
4. **Deploy**: the existing daily cron already deploys via
   ...
   deploy will pick up the new `api/` functions automatically (Vercel Python
   runtime is detected from the `api/*.py` files — no `vercel.json` required for
   basic routing).
...
his own browser. It does not redistribute Kalshi data to third parties (token +
origin gate restrict to Andrew's browser). It uses only lightweight
authentication (a shared secret), which is permitted. It does not use, store, or
proxy Kalshi's authenticated (trading) API. Live order book / settlement data
shown is the same data Kalshi publishes on its own website; this surface just
makes it reachable from the browser by relocating the fetch server-side.
